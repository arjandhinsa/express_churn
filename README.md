...
The notebooks run top-to-bottom from a clean kernel, with all paths anchored to the project root and a fixed random seed. Because the underlying data is real patient data, this repository documents the work rather than shipping the data itself. A planned extension is a **synthetic data generator** producing VisionPlus-shaped records, enabling the full pipeline to be run end-to-end by anyone, and a refactor of the label, feature, and modelling logic from notebooks into importable `src/` modules with a single-command pipeline entry point.
